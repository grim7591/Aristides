...
result = pd.read_csv('Data/dp74m.csv')
...
# 1) Convert 'prop_id' to string and build sets for fast membership checks
excl_2021_set = set(XXI_exclusions_f2['prop_id'].astype(str))
excl_2022_set = set(XXII_exclusions_f2['prop_id'].astype(str))
excl_2023_set = set(XXIII_exclusions_f2['prop_id'].astype(str))
excl_2024_set = set(XXIV_exclusions_f2['prop_id'].astype(str))

# 2) Map each sale_year to the appropriate exclusions set
...
# 3) Define a function that returns True if the row should be *excluded*
def is_excluded(row):
    """
    Returns True if row's sale_year is in the dictionary 
    AND row['prop_id'] is found in that year's exclusion set.
    """
    sy = row['sale_year']
...
# 4) Filter out excluded rows
# Keep only rows where is_excluded(row) == False.
result = result[~result.apply(is_excluded, axis=1)]

print("Exclusions applied. 'result' now has rows not in the year-matched exclusion tables.")
...
test_size_var = 0.2
train_data, test_data = train_test_split(result, test_size=test_size_var, random_state=44)
